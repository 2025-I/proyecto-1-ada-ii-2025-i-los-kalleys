...
def generate_tree_solution(size: int) -> tuple[list[list[int]], list[int], list[int]]:
    ...
    best_independent_set = [1 if node in clique else 0 for node in tree.nodes()]
    best_independent_set.append(total_sum)

    return adj_matrix, weights, best_independent_set
